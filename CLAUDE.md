...
- `pgsql.php` – two arguments: a checkout of the latest stable branch (`REL_*_STABLE`) of https://github.com/postgres/postgres and a checkout of https://github.com/cockroachdb/docs; the `pgsqlext` extension links come from the `*.control` files in `contrib/` and `src/pl/`, only their full URLs (PostGIS, TimescaleDB, … and the PGXN fallback last) are maintained by hand; the lists in `jush.link_key.pgsql` are the statements and types with a CockroachDB page of the same title (or `key`) in the newest `src/current/v*` directory, linked when Adminer replaces the base URL
...
- `oracle.php` – a cache directory for the table of contents of the Oracle SQL Language Reference (fetched from docs.oracle.com on miss; bump `$version` in the script for a new release). The pages are named after the construct, so `jush.slugs.oracle` derives nearly every key and almost the whole module merges into one `'$1.html'` entry. It works like `mssql.php` – re-key, merge, order, report – except that the names come from the table of contents instead of a front matter, and a key it cannot derive is kept only when the page still exists
