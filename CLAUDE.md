...
- `mssql.php` – a checkout of https://github.com/MicrosoftDocs/sql-docs; entry keys are the path of the page below `docs/` without `.md`, the address of `https://learn.microsoft.com/sql/$key`. It only re-keys the hand-written entries by matching their phrases against the `title` and `f1_keywords` front matter, never their regexps; a key it can't derive is kept and reported, a full URL is left alone as hand-maintained. It also reports the documented constructs which are not linked yet – acting on those is a separate change
