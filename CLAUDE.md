...
- `pgsql.php` – a checkout of the latest stable branch (`REL_*_STABLE`) of https://github.com/postgres/postgres; the `pgsqlext` extension links come from the `*.control` files in `contrib/` and `src/pl/`, only their full URLs (PostGIS, TimescaleDB, … and the PGXN fallback last) are maintained by hand
