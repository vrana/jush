				...
				for (var i=arguments.length - 4; i > 0; i--) {
					if (arguments[i]) {
						...
						switch (state) {
							...
							case 'sql_set': link = link.replace(/\$1/g, (links2.test(arguments[i].replace('_', '-')) ? arguments[i].replace('_', '-') : arguments[i]).toLowerCase()); break;
...
jush.urls = {
	...
	//! sql_set: 'http://dev.mysql.com/doc/mysql/en/set-option.html',
	...
	sql_set: [ 'http://dev.mysql.com/doc/mysql/en/$key',
		...
		'server-system-variables.html#sysvar_low_priority_updates', 'server-system-variables.html#sysvar_max_join_size', 'server-system-variables.html#sysvar_storage_engine', 'server-system-variables.html#sysvar_$1',
