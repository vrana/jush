			...
			for (var key in tr[state]) {
				...
				if (m && !m.index && m[0].length == match[0].length) { // check index and length to allow '/' before '</script>'
					...
					if (/^(att_quo|att_apo|att_val)$/.test(state) && (/^(att_js|att_css|att_http)$/.test(prev_state) || /^\s*javascript:/i.test(s))) { // javascript: - easy but without own state //! should be checked only in %URI;
						child_states.unshift(prev_state == 'att_css' ? 'css_pro' : prev_state.substr(4));
						...
						if (/^(att_js|att_css|att_http)$/.test(prev_state)) {
							...
							s_states = [ (escape ? escape(s) : s), (isNaN(+key) || !/^(att_js|att_css|att_http|css_js|js_write_code|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini|php_http)$/.test(state) || /^(js_write_code|php_echo|php_sql|php_sqlite|php_pgsql|php_phpini|php_http|css_js)$/.test(prev_state) ? child_states : [ ]) ];
						}
					} else {
						s = this.htmlspecialchars(s);
						s_states = [ (escape ? escape(s) : s), (isNaN(+key) || !/^(att_js|att_css|att_http|css_js|js_write_code|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini|php_http)$/.test(state) || /^(js_write_code|php_echo|php_sql|php_sqlite|php_pgsql|php_phpini|php_http|css_js)$/.test(prev_state) ? child_states : [ ]) ]; // reset child states when escaping construct
					}
					s = s_states[0];
...
jush.urls = {
	...
	sql: [ 'http://dev.mysql.com/doc/mysql/en/$key',
		...
		'comparison-operators.html#operator_$1', 'comparison-operators.html#function_$1', 'any-in-some-subqueries.html', 'all-subqueries.html', 'exists-and-not-exists-subqueries.html', 'group-by-modifiers.html', 'string-comparison-functions.html#operator_$1', 'logical-operators.html#operator_$1', 'control-flow-functions.html#operator_$1', 'arithmetic-functions.html#operator_$1', 'cast-functions.html#operator_$1', 'date-and-time-functions.html#function_date-add',
