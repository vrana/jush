...
var jush = {
	...
	keywords_links: function (state, s) {
		...
		if (/^(php_quo_var|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini|php_http|php_mail)$/.test(state)) {
			state = 'php';
		}
	...
	highlight_states: function (states, text, in_php, escape) {
		...
		var tr = { // transitions
			...
			php_mail: { php_quo: /"/, php_apo: /'/, php_bac: /`/, php_one: /\/\/|#/, php_com: /\/\*/, php_eot: /<<<[ \t]*/, php_mail: /\(/, php_var: /\$/, num: num, 1: /\)/ },
		...
		loop: while (start < text.length && (match = regexps[state].exec(text))) {
			for (var key in tr[state]) {
				...
				if (m && !m.index && m[0].length == match[0].length) { // check index and length to allow '/' before '</script>'
					...
					if (/^(att_quo|att_apo|att_val)$/.test(state) && (/^(att_js|att_css|att_http)$/.test(prev_state) || /^\s*javascript:/i.test(s))) { // javascript: - easy but without own state //! should be checked only in %URI;
					...
					} else if ((state == 'php_quo' || state == 'php_apo') && /^(php_sql|php_sqlite|php_pgsql|php_phpini|php_http|php_mail)$/.test(prev_state)) {
						...
						if (/^(att_js|att_css|att_http)$/.test(prev_state)) {
							...
							s_states = [ (escape ? escape(s) : s), (isNaN(+key) || !/^(att_js|att_css|att_http|css_js|js_write_code|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini|php_http|php_mail)$/.test(state) ? child_states : [ ]) ];
						}
					} else {
						s = this.htmlspecialchars(s);
						s_states = [ (escape ? escape(s) : s), (isNaN(+key) || !/^(att_js|att_css|att_http|css_js|js_write_code|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini|php_http|php_mail)$/.test(state) ? child_states : [ ]) ]; // reset child states when escaping construct
					}
					s = s_states[0];
...
jush.urls = {
	...
	http: [ 'http://www.w3.org/Protocols/rfc2616/rfc2616-$key',
		...
		'http://www.ietf.org/rfc/rfc2109.txt', 'http://en.wikipedia.org/wiki/$1', 'http://msdn.microsoft.com/library/cc288472.aspx#_replace', 'http://msdn.microsoft.com/library/cc817574.aspx'
	],
