...
jush.api.php = {
	...
	ascii2ebcdic: 'Translate string from ASCII to EBCDIC (string ascii_str): int',
	ebcdic2ascii: 'Translate string from EBCDIC to ASCII (string ebcdic_str): int',
	...
	apc_fetch: 'Fetch a stored variable from the cache (string key, [bool &success]): mixed',
	...
	array_reduce: 'Iteratively reduce the array to a single value using a callback function (array input, callback function, [int initial]): mixed',
	...
	list: 'Assign variables as if they were an array (mixed varname): null',
	...
	date_offset_get: 'Returns the daylight saving time offset (DateTime object): int',
	...
	timezone_open: 'Returns new DateTimeZone object (string timezone): DateTimeZone',
	...
	dio_open: 'Opens a new filename with specified permissions of flags and creation permissions of mode (string filename, int flags, [int mode]): resource',
	...
	set_exception_handler: 'Sets a user-defined exception handler function (callback exception_handler): string',
	...
	disk_free_space: 'Returns available space in directory (string directory): float',
	disk_total_space: 'Returns the total size of a directory (string directory): float',
	...
	file_get_contents: 'Reads entire file into a string (string filename, [int flags, [resource context, [int offset, [int maxlen]]]]): string',
	...
	is_readable: 'Tells whether the filename is readable (string filename): bool',
	...
	link: 'Create a hard link (string to_path, string from_path): bool',
	...
	imagegrabwindow: 'Captures a window (int window, [int client_area]): resource',
	...
	imap_append: 'Append a string message to a specified mailbox (resource imap_stream, string mailbox, string message, [string options]): bool',
	...
	imap_open: 'Open an IMAP stream to a mailbox (string mailbox, string username, string password, [int options, [int n_retries]]): resource',
	...
	dl: 'Loads a PHP extension at runtime (string library): int',
	...
	phpinfo: 'Outputs lots of PHP information ([int what]): bool',
	...
	event_buffer_base_set: 'Associate buffered event with an event base (resource bevent, resource event_base): resource',
	...
	event_buffer_priority_set: 'Assign a priority to a buffered event (resource bevent, int priority): resource',
	event_buffer_read: 'Read data from a buffered event (resource bevent,  data_size): string',
	...
	event_buffer_watermark_set: 'Set the watermarks for read and write events (resource bevent, int events, int lowmark, int highmark): bool',
	...
	event_free: 'Free event resource (resource event): bool',
	...
	addfill: 'Adds a solid fill to the shape (SWFBitmap bitmap, [int flags]): SWFFill',
	addfill: 'Adds a solid fill to the shape (SWFGradient gradient, [int flags]): SWFFill',
	setleftfill: 'Sets left rasterizing color (int red, int green, int blue, [int a]): null',
	setline: 'Sets the shape\'s line style (int width, int red, int green, int blue, [int a]): null',
	setrightfill: 'Sets right rasterizing color (int red, int green, int blue, [int a]): null',
	...
	mysql_change_user: 'Change logged in user of the active connection (string user, string password, [string database, [resource link_identifier]]): int',
	...
	mysql_insert_id: 'Get the ID generated from the previous INSERT operation ([resource link_identifier]): int',
	...
	mysqli_get_warnings: 'Get result of SHOW WARNINGS (mysqli link): object',
	...
	oauth_urlencode: 'Encode a URI to RFC3686 (string uri): string',
	...
	oci_bind_by_name: 'Binds the PHP variable to the Oracle placeholder (resource statement, string ph_name, mixed &variable, [int maxlength, [int type]]): bool',
	oci_cancel: 'Cancels reading from cursor (resource statement): bool',
	oci_close: 'Closes Oracle connection (resource connection): bool',
	oci_commit: 'Commits outstanding statements (resource connection): bool',
	oci_connect: 'Establishes a connection to the Oracle server (string username, string password, [string db, [string charset, [int session_mode]]]): resource',
	oci_define_by_name: 'Uses a PHP variable for the define-step during a SELECT (resource statement, string column_name, mixed &variable, [int type]): bool',
	oci_error: 'Returns the last error found ([resource source]): array',
	oci_execute: 'Executes a statement (resource statement, [int mode]): bool',
	oci_fetch_all: 'Fetches all rows of result data into an array (resource statement, array &output, [int skip, [int maxrows, [int flags]]]): int',
	oci_fetch_array: 'Returns the next row from the result data as an associative or numeric array, or both (resource statement, [int mode]): array',
	oci_fetch_assoc: 'Returns the next row from the result data as an associative array (resource statement): array',
	oci_fetch_object: 'Returns the next row from the result data as an object (resource statement): object',
	oci_fetch_row: 'Returns the next row from the result data as a numeric array (resource statement): array',
	oci_fetch: 'Fetches the next row into result-buffer (resource statement): bool',
	...
	oci_new_connect: 'Establishes a new connection to the Oracle server (string username, string password, [string db, [string charset, [int session_mode]]]): resource',
	...
	oci_parse: 'Prepares Oracle statement for execution (resource connection, string query): resource',
	...
	oci_pconnect: 'Connect to an Oracle database using a persistent connection (string username, string password, [string db, [string charset, [int session_mode]]]): resource',
	oci_result: 'Returns field\'s value from the fetched row (resource statement, mixed field): mixed',
	oci_rollback: 'Rolls back outstanding transaction (resource connection): bool',
	oci_server_version: 'Returns server version (resource connection): string',
	oci_set_prefetch: 'Sets number of rows to be prefetched (resource statement, int rows): bool',
	oci_statement_type: 'Returns the type of an OCI statement (resource statement): string',
	...
	openssl_random_pseudo_bytes: 'Generate a pseudo-random string (string length, string strong): bool',
	...
	rar_close: 'Close Rar archive and free all resources (resource rar_file): bool',
	rar_entry_get: 'Get entry object from the Rar archive (resource rar_file, string entry_name): RarEntry',
	rar_list: 'Get entries list from the Rar archive (resource rar_file): array',
	rar_open: 'Open Rar archive (string filename, [string password]): resource',
	...
	iterator_apply: 'Apply a user function to every element of an iterator (Traversable iterator, callback function, [array args]): int',
	...
	spl_autoload_register: 'Register given function as __autoload() implementation ([callback autoload_function]): bool',
	...
	stats_rand_gen_ibinomial_negative: 'Generates a single random deviate from a negative binomial distribution. Arguments : n - the number of trials in the negative binomial distribution from which a random deviate is to be generated (n  (int n, float p)',
	stats_rand_gen_ibinomial: 'Generates a single random deviate from a binomial distribution whose number of trials is "n" (n  (int n, float pp)',
	stats_rand_gen_int: 'Generates random integer between 1 and 2147483562 (): int',
	stats_rand_gen_ipoisson: 'Generates a single random deviate from a Poisson distribution with mean "mu" (mu  (float mu)',
	...
	stats_rand_gen_noncenral_chisquare: 'Generates random deviate from the distribution of a noncentral chisquare with "df" degrees of freedom and noncentrality parameter "xnonc". d must be  (float df, float xnonc)',
	...
	stats_rand_gen_normal: 'Generates a single random deviate from a normal distribution with mean, av, and standard deviation, sd (sd  (float av, float sd)',
	...
	stream_notification_callback: 'A callback function for the notification context paramater (int notification_code, int severity, string message, int message_code, int bytes_transferred, int bytes_max): null',
	...
	crypt: 'One-way string encryption (hashing) (string str, [string salt]): string',
	...
	metaphone: 'Calculate the metaphone key of a string (string str, [int phones]): string',
	...
	strspn: 'Find length of initial segment matching mask (string str1, string str2, [int start, [int length]]): int',
	...
	substr_compare: 'Binary safe comparison of 2 strings from an offset, up to length characters (string main_str, string str, int offset, [int length, [bool case_insensitivity]]): int',
	...
	tidy_get_body: 'Returns a tidyNode object starting from the <body (tidy object)',
	...
	tidy_get_head: 'Returns a tidyNode object starting from the <head (tidy object)',
	tidy_get_html: 'Returns a tidyNode object starting from the <html (tidy object)',
