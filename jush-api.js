...
jush.api.js = {
	'AbstractModuleSource': 'An _AbstractModuleSource_ object represents the compiled source of a module obtained with import source or import.source(). There is no directly visible AbstractModuleSource constructor.',
	...
	'Array.fromAsync': 'The Array.fromAsync() static method creates a new, shallow-copied Array instance from an async iterable, iterable, or array-like object.',
	...
	'ArrayBuffer': 'The ArrayBuffer object is used to represent a generic raw binary data buffer.',
	'ArrayBuffer.isView': 'The ArrayBuffer.isView() static method determines whether the passed value is one of the ArrayBuffer views, such as typed array objects or a DataView.',
	...
	'AsyncFunction': 'The AsyncFunction object provides methods for async functions. In JavaScript, every async function is actually an AsyncFunction object.',
	'AsyncGenerator': 'The AsyncGenerator object is returned by an async generator function and it conforms to both the async iterable protocol and the async iterator protocol.',
	...
	'Atomics': 'The Atomics namespace object contains static methods for carrying out atomic operations. They are used with SharedArrayBuffer and ArrayBuffer objects.',
	'Atomics.add': 'The Atomics.add() static method adds a given value at a given position in the array, and returns the old value at that position. This atomic operation guarantees that no other write happens until the modified value is written back.',
	'Atomics.and': 'The Atomics.and() static method computes a bitwise AND with a given value at a given position in the array, and returns the old value at that position. This atomic operation guarantees that no other write happens until the modified value is written back.',
	...
	'Atomics.isLockFree': 'The Atomics.isLockFree() static method is used to determine whether the Atomics methods use locks or atomic hardware operations when applied to typed arrays with the given element byte size. It is intended as an optimization primitive, so that high-performance algorithms can determine whether to use locks or atomic operations in critical sections. If an atomic primitive is not lock-free, it is often more efficient for an algorithm to provide its own locking.',
	'Atomics.load': 'The Atomics.load() static method returns a value at a given position in the array. This atomic operation guarantees that the read is tear-free, and that all atomic reads are sequentially consistent.',
	'Atomics.notify': 'The Atomics.notify() static method notifies up some agents that are sleeping in the wait queue.',
	'Atomics.or': 'The Atomics.or() static method computes a bitwise OR with a given value at a given position in the array, and returns the old value at that position. This atomic operation guarantees that no other write happens until the modified value is written back.',
	'Atomics.pause': 'The Atomics.pause() static method provides a micro-wait primitive that hints to the CPU that the caller is spinning while waiting on access to a shared resource. This allows the system to reduce the resources allocated to the core (such as power) or thread, without yielding the current thread.',
	'Atomics.store': 'The Atomics.store() static method stores a given value at a given position in the array and returns that value. This atomic operation guarantees that the write is tear-free, and that all atomic writes are sequentially consistent.',
	'Atomics.sub': 'The Atomics.sub() static method subtracts a given value at a given position in the array, and returns the old value at that position. This atomic operation guarantees that no other write happens until the modified value is written back.',
	'Atomics.wait': 'The Atomics.wait() static method verifies that a shared memory location contains a given value and if so sleeps, awaiting a wake-up notification or a timeout. It returns a string which is "not-equal" if the memory location does not match the given value, "ok" if woken by Atomics.notify(), or "timed-out" if the timeout expires.',
	'Atomics.waitAsync': 'The Atomics.waitAsync() static method verifies that a shared memory location contains a given value, immediately returning an object with the value property containing the string "not-equal" if the memory location does not match the given value, or "timed-out" if the timeout was set to zero. Otherwise the method returns an object where the value property is a Promise that fulfills with either "ok" when Atomics.notify() is called, or "timed-out" if the timeout expires.',
	'Atomics.xor': 'The Atomics.xor() static method computes a bitwise XOR with a given value at a given position in the array, and returns the old value at that position. This atomic operation guarantees that no other write happens until the modified value is written back.',
	'BigInt': 'BigInt values represent integer values which are too high or too low to be represented by the number primitive.',
	'BigInt.asIntN': 'The BigInt.asIntN() static method truncates a BigInt value to the given number of least significant bits and returns that value as a signed integer.',
	'BigInt.asUintN': 'The BigInt.asUintN() static method truncates a BigInt value to the given number of least significant bits and returns that value as an unsigned integer.',
	...
	'BigUint64Array': 'The BigUint64Array typed array represents an array of 64-bit unsigned integers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0n unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Boolean': 'Boolean values can be one of two values: true or false, representing the truth value of a logical proposition.',
	...
	'Date': 'JavaScript Date objects represent a single moment in time in a platform-independent format. Date objects encapsulate an integral number that represents milliseconds since the midnight at the beginning of January 1, 1970, UTC (the _epoch_).',
	'Date.UTC': 'The Date.UTC() static method accepts parameters representing the date and time components similar to the Date constructor, but treats them as UTC. It returns the number of milliseconds since January 1, 1970, 00:00:00 UTC.',
	...
	'Date.parse': 'The Date.parse() static method parses a string representation of a date, and returns the date\'s timestamp.',
	'DisposableStack': 'The DisposableStack object represents a stack of disposers to run when the stack itself is disposed. Disposer functions are executed in reverse order of registration, with strong error handling guarantees. Calling its move() method will transfer responsibility for calling the current registered disposers to a new DisposableStack and prevent registering any additional disposers.',
	'Error': 'Error objects are thrown when runtime errors occur. The Error object can also be used as a base object for user-defined exceptions. See below for standard built-in error types.',
	'Error.captureStackTrace': 'The Error.captureStackTrace() static method installs stack trace information on a provided object as the stack property.',
	'Error.isError': 'The Error.isError() static method determines whether the passed value is an Error.',
	'Error.stackTraceLimit': 'The Error.stackTraceLimit static data property indicates the maximum number of stack frames captured by the stack trace of an error. It can be set by user code to change the engine\'s behavior.',
	...
	'FinalizationRegistry': 'A FinalizationRegistry object lets you request a callback when a value is garbage-collected.',
	'Float16Array': 'The Float16Array typed array represents an array of 16-bit floating point numbers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Float32Array': 'The Float32Array typed array represents an array of 32-bit floating point numbers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Float64Array': 'The Float64Array typed array represents an array of 64-bit floating point numbers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	...
	'Infinity': 'The Infinity global property is a numeric value representing infinity.',
	'Int16Array': 'The Int16Array typed array represents an array of 16-bit signed integers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Int32Array': 'The Int32Array typed array represents an array of 32-bit signed integers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Int8Array': 'The Int8Array typed array represents an array of 8-bit signed integers. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	...
	'Intl.getCanonicalLocales': 'The Intl.getCanonicalLocales() static method returns an array containing the canonical locale names. Duplicates will be omitted and elements will be validated as structurally valid language tags.',
	...
	'Iterator.concat': 'The Iterator.concat() static method creates a new Iterator object from a list of iterable objects. The new iterator yields the values from each of the input iterables in sequence.',
	'Iterator.from': 'The Iterator.from() static method creates a new Iterator object from an iterator or iterable object.',
	'Iterator.zip': 'The Iterator.zip() static method creates a new Iterator object that aggregates elements from multiple iterable objects by yielding arrays containing elements at the same position. It essentially "zips" the input iterables together, allowing simultaneous iteration over them.',
	'Iterator.zipKeyed': 'The Iterator.zipKeyed() static method creates a new Iterator object that aggregates elements from multiple iterable objects by yielding objects containing elements at the same position, with keys specified by the input. It essentially "zips" the input iterables together, allowing simultaneous iteration over them.',
	...
	'JSON.isRawJSON': 'The JSON.isRawJSON() static method tests whether a value is an object returned by JSON.rawJSON().',
	'JSON.parse': 'The JSON.parse() static method parses a JSON string, constructing the JavaScript value or object described by the string. An optional _reviver_ function can be provided to perform a transformation on the resulting object before it is returned.',
	'JSON.rawJSON': 'The JSON.rawJSON() static method creates a "raw JSON" object containing a piece of JSON text. When serialized to JSON, the raw JSON object is treated as if it is already a piece of JSON. This text is required to be valid JSON.',
	...
	'Map.groupBy': 'The Map.groupBy() static method groups the elements of a given iterable using the values returned by a provided callback function. The final returned Map uses the unique values from the test function as keys, which can be used to get the array of elements in each group.',
	'Math': 'The Math namespace object contains static properties and methods for mathematical constants and functions.',
	'Math.E': 'The Math.E static data property represents Euler\'s number, the base of natural logarithms, e, which is approximately 2.718.',
	'Math.LN10': 'The Math.LN10 static data property represents the natural logarithm of 10, approximately 2.303.',
	'Math.LN2': 'The Math.LN2 static data property represents the natural logarithm of 2, approximately 0.693:',
	'Math.LOG10E': 'The Math.LOG10E static data property represents the base 10 logarithm of e, approximately 0.434.',
	'Math.LOG2E': 'The Math.LOG2E static data property represents the base 2 logarithm of e, approximately 1.443.',
	...
	'Math.SQRT1_2': 'The Math.SQRT1_2 static data property represents the square root of 1/2, which is approximately 0.707.',
	'Math.SQRT2': 'The Math.SQRT2 static data property represents the square root of 2, approximately 1.414.',
	'Math.abs': 'The Math.abs() static method returns the absolute value of a number.',
	'Math.acos': 'The Math.acos() static method returns the inverse cosine (in radians) of a number. That is,',
	'Math.acosh': 'The Math.acosh() static method returns the inverse hyperbolic cosine of a number. That is,',
	'Math.asin': 'The Math.asin() static method returns the inverse sine (in radians) of a number. That is,',
	'Math.asinh': 'The Math.asinh() static method returns the inverse hyperbolic sine of a number. That is,',
	'Math.atan': 'The Math.atan() static method returns the inverse tangent (in radians) of a number, that is',
	'Math.atan2': 'The Math.atan2() static method returns the angle in the plane (in radians) between the positive x-axis and the ray from (0, 0) to the point (x, y), for Math.atan2(y, x).',
	'Math.atanh': 'The Math.atanh() static method returns the inverse hyperbolic tangent of a number. That is,',
	'Math.cbrt': 'The Math.cbrt() static method returns the cube root of a number. That is',
	...
	'Math.clz32': 'The Math.clz32() static method returns the number of leading zero bits in the 32-bit binary representation of a number.',
	'Math.cos': 'The Math.cos() static method returns the cosine of a number in radians.',
	'Math.cosh': 'The Math.cosh() static method returns the hyperbolic cosine of a number. That is,',
	'Math.exp': 'The Math.exp() static method returns e raised to the power of a number. That is',
	'Math.expm1': 'The Math.expm1() static method returns e raised to the power of a number, subtracted by 1. That is',
	'Math.f16round': 'The Math.f16round() static method returns the nearest 16-bit half precision float representation of a number.',
	...
	'Math.fround': 'The Math.fround() static method returns the nearest 32-bit single precision float representation of a number.',
	'Math.hypot': 'The Math.hypot() static method returns the square root of the sum of squares of its arguments. That is,',
	'Math.imul': 'The Math.imul() static method returns the result of the C-like 32-bit multiplication of the two parameters.',
	'Math.log': 'The Math.log() static method returns the natural logarithm (base e) of a number. That is',
	'Math.log10': 'The Math.log10() static method returns the base 10 logarithm of a number. That is',
	'Math.log1p': 'The Math.log1p() static method returns the natural logarithm (base e) of 1 + x, where x is the argument. That is:',
	'Math.log2': 'The Math.log2() static method returns the base 2 logarithm of a number. That is',
	'Math.max': 'The Math.max() static method returns the largest of the numbers given as input parameters, or -Infinity if there are no parameters.',
	'Math.min': 'The Math.min() static method returns the smallest of the numbers given as input parameters, or Infinity if there are no parameters.',
	'Math.pow': 'The Math.pow() static method returns the value of a base raised to a power. That is',
	...
	'Math.sign': 'The Math.sign() static method returns 1 or -1, indicating the sign of the number passed as argument. If the input is 0 or -0, it will be returned as-is.',
	'Math.sin': 'The Math.sin() static method returns the sine of a number in radians.',
	'Math.sinh': 'The Math.sinh() static method returns the hyperbolic sine of a number. That is,',
	'Math.sqrt': 'The Math.sqrt() static method returns the square root of a number. That is',
	'Math.sumPrecise': 'The Math.sumPrecise() static method takes an iterable of numbers and returns the sum of them. It is more precise than summing them up in a loop, because it avoids floating point precision loss in intermediate results.',
	'Math.tan': 'The Math.tan() static method returns the tangent of a number in radians.',
	'Math.tanh': 'The Math.tanh() static method returns the hyperbolic tangent of a number. That is,',
	...
	'Number.MAX_SAFE_INTEGER': 'The Number.MAX_SAFE_INTEGER static data property represents the maximum safe integer in JavaScript (2^53 – 1).',
	'Number.MAX_VALUE': 'The Number.MAX_VALUE static data property represents the maximum numeric value representable in JavaScript.',
	'Number.MIN_SAFE_INTEGER': 'The Number.MIN_SAFE_INTEGER static data property represents the minimum safe integer in JavaScript, or -(2^53 - 1).',
	'Number.MIN_VALUE': 'The Number.MIN_VALUE static data property represents the smallest positive numeric value representable in JavaScript.',
	'Number.NEGATIVE_INFINITY': 'The Number.NEGATIVE_INFINITY static data property represents the negative Infinity value.',
	'Number.NaN': 'The Number.NaN static data property represents Not-A-Number, which is equivalent to NaN. For more information about the behaviors of NaN, see the description for the global property.',
	'Number.POSITIVE_INFINITY': 'The Number.POSITIVE_INFINITY static data property represents the positive Infinity value.',
	'Number.isFinite': 'The Number.isFinite() static method determines whether the passed value is a finite number — that is, it checks that a given value is a number, and the number is neither positive Infinity, negative Infinity, nor NaN.',
	'Number.isInteger': 'The Number.isInteger() static method determines whether the passed value is an integer.',
	'Number.isNaN': 'The Number.isNaN() static method determines whether the passed value is the number value NaN, and returns false if the input is not of the Number type. It is a more robust version of the original, global isNaN() function.',
	'Number.isSafeInteger': 'The Number.isSafeInteger() static method determines whether the provided value is a number that is a _safe integer_.',
	...
	'Number.parseInt': 'The Number.parseInt() static method parses a string argument and returns an integer of the specified radix or base.',
	...
	'Object.assign': 'The Object.assign() static method copies all enumerable own properties from one or more _source objects_ to a _target object_. It returns the modified target object.',
	...
	'Object.defineProperties': 'The Object.defineProperties() static method defines new or modifies existing properties directly on an object, returning the object.',
	...
	'Object.getOwnPropertyDescriptors': 'The Object.getOwnPropertyDescriptors() static method returns all own property descriptors of a given object.',
	'Object.getOwnPropertyNames': 'The Object.getOwnPropertyNames() static method returns an array of all properties (including non-enumerable properties except for those which use Symbol) found directly in a given object.',
	'Object.getOwnPropertySymbols': 'The Object.getOwnPropertySymbols() static method returns an array of all symbol properties found directly upon a given object.',
	'Object.getPrototypeOf': 'The Object.getPrototypeOf() static method returns the prototype (i.e., the value of the internal [[Prototype]] property) of the specified object.',
	'Object.groupBy': 'The Object.groupBy() static method groups the elements of a given iterable according to the string values returned by a provided callback function. The returned object has separate properties for each group, containing arrays with the elements in the group.',
	'Object.hasOwn': 'The Object.hasOwn() static method returns true if the specified object has the indicated property as its _own_ property. If the property is inherited, or does not exist, the method returns false.',
	'Object.is': 'The Object.is() static method determines whether two values are the same value.',
	'Object.isExtensible': 'The Object.isExtensible() static method determines if an object is extensible (whether it can have new properties added to it).',
	'Object.isFrozen': 'The Object.isFrozen() static method determines if an object is frozen.',
	'Object.isSealed': 'The Object.isSealed() static method determines if an object is sealed.',
	...
	'Object.preventExtensions': 'The Object.preventExtensions() static method prevents new properties from ever being added to an object (i.e., prevents future extensions to the object). It also prevents the object\'s prototype from being re-assigned.',
	...
	'Object.setPrototypeOf': 'The Object.setPrototypeOf() static method sets the prototype (i.e., the internal [[Prototype]] property) of a specified object to another object or null.',
	...
	'Promise.try': 'The Promise.try() static method takes a callback of any kind (returns or throws, synchronously or asynchronously) and wraps its result in a Promise.',
	'Promise.withResolvers': 'The Promise.withResolvers() static method returns an object containing a new Promise object and two functions to resolve or reject it, corresponding to the two parameters passed to the executor of the Promise() constructor.',
	...
	'Proxy.revocable': 'The Proxy.revocable() static method creates a revocable Proxy object.',
	...
	'Reflect': 'The Reflect namespace object contains static methods for invoking interceptable JavaScript object internal methods. The methods are the same as those of proxy handlers.',
	'Reflect.apply': 'The Reflect.apply() static method calls a target function with arguments as specified.',
	'Reflect.construct': 'The Reflect.construct() static method is like the new operator, but as a function. It is equivalent to calling new target(...args). It additionally allows you to specify a different new.target value.',
	...
	'RegExp': 'The RegExp object is used for matching text with a pattern.',
	'RegExp.escape': 'The RegExp.escape() static method escapes any potential regex syntax characters in a string, and returns a new string that can be safely used as a literal pattern for the RegExp() constructor.',
	'RegExp.input': 'The RegExp.input static accessor property returns the string against which a regular expression is matched. RegExp.$_ is an alias for this property.',
	'RegExp.lastMatch': 'The RegExp.lastMatch static accessor property returns the last matched substring. RegExp["$&"] is an alias for this property.',
	'RegExp.lastParen': 'The RegExp.lastParen static accessor property returns the last parenthesized substring match, if any. RegExp["$+"] is an alias for this property.',
	'RegExp.leftContext': 'The RegExp.leftContext static accessor property returns the substring preceding the most recent match. RegExp["$"] is an alias for this property.',
	'RegExp.n': 'The RegExp.$1, …, RegExp.$9 static accessor properties return parenthesized substring matches.',
	'RegExp.rightContext': 'The RegExp.rightContext static accessor property returns the substring following the most recent match. RegExp["$\'"] is an alias for this property.',
	'Set': 'The Set object lets you store unique values of any type, whether primitive values or object references.',
	'SharedArrayBuffer': 'The SharedArrayBuffer object is used to represent a generic raw binary data buffer, similar to the ArrayBuffer object, but in a way that they can be used to create views on shared memory. A SharedArrayBuffer is not a Transferable Object, unlike an ArrayBuffer which is transferable.',
	'String': 'The String object is used to represent and manipulate a sequence of characters.',
	'String.fromCharCode': 'The String.fromCharCode() static method returns a string created from the specified sequence of UTF-16 code units.',
	'String.fromCodePoint': 'The String.fromCodePoint() static method returns a string created from the specified sequence of code points.',
	'String.raw': 'The String.raw() static method is a tag function of template literals. This is similar to the r prefix in Python, or the @ prefix in C# for string literals. It\'s used to get the raw string form of template literals — that is, substitutions (e.g., ${foo}) are processed, but escape sequences (e.g., \\n) are not.',
	'SuppressedError': 'The SuppressedError object represents an error generated while handing another error. It is generated during resource disposal using using or await using.',
	...
	'Symbol.asyncDispose': 'The Symbol.asyncDispose static data property represents the well-known symbol Symbol.asyncDispose. The await using declaration looks up this symbol on the variable initializer for the method to call when the variable goes out of scope.',
	'Symbol.asyncIterator': 'The Symbol.asyncIterator static data property represents the well-known symbol Symbol.asyncIterator. The async iterable protocol looks up this symbol for the method that returns the async iterator for an object. In order for an object to be async iterable, it must have a [Symbol.asyncIterator] key.',
	'Symbol.dispose': 'The Symbol.dispose static data property represents the well-known symbol Symbol.dispose. The using declaration looks up this symbol on the variable initializer for the method to call when the variable goes out of scope.',
	'Symbol.for': 'The Symbol.for() static method searches for existing symbols in a runtime-wide symbol registry with the given key and returns it if found. Otherwise a new symbol gets created in the global symbol registry with this key.',
	'Symbol.hasInstance': 'The Symbol.hasInstance static data property represents the well-known symbol Symbol.hasInstance. The instanceof operator looks up this symbol on its right-hand operand for the method used to determine if the constructor object recognizes an object as its instance.',
	'Symbol.isConcatSpreadable': 'The Symbol.isConcatSpreadable static data property represents the well-known symbol Symbol.isConcatSpreadable. The Array.prototype.concat() method looks up this symbol on each object being concatenated to determine if it should be treated as an array-like object and flattened to its array elements.',
	'Symbol.iterator': 'The Symbol.iterator static data property represents the well-known symbol Symbol.iterator. The iterable protocol looks up this symbol for the method that returns the iterator for an object. In order for an object to be iterable, it must have a [Symbol.iterator] key.',
	'Symbol.keyFor': 'The Symbol.keyFor() static method retrieves a shared symbol key from the global symbol registry for the given symbol.',
	'Symbol.match': 'The Symbol.match static data property represents the well-known symbol Symbol.match. The String.prototype.match() method looks up this symbol on its first argument for the method used to match an input string against the current object. This symbol is also used to determine if an object should be treated as a regex.',
	'Symbol.matchAll': 'The Symbol.matchAll static data property represents the well-known symbol Symbol.matchAll. The String.prototype.matchAll() method looks up this symbol on its first argument for the method that returns an iterator, that yields matches of the current object against a string.',
	'Symbol.replace': 'The Symbol.replace static data property represents the well-known symbol Symbol.replace. The String.prototype.replace() and String.prototype.replaceAll() methods look up this symbol on their first argument for the method that replaces substrings matched by the current object.',
	'Symbol.search': 'The Symbol.search static data property represents the well-known symbol Symbol.search. The String.prototype.search() method looks up this symbol on its first argument for the method that returns the index within a string that matches the current object.',
	'Symbol.species': 'The Symbol.species static data property represents the well-known symbol Symbol.species. Methods that create copies of an object may look up this symbol on the object for the constructor function to use when creating the copy.',
	'Symbol.split': 'The Symbol.split static data property represents the well-known symbol Symbol.split. The String.prototype.split() method looks up this symbol on its first argument for the method that splits a string at the indices that match the current object.',
	'Symbol.toPrimitive': 'The Symbol.toPrimitive static data property represents the well-known symbol Symbol.toPrimitive. All type coercion algorithms look up this symbol on objects for the method that accepts a preferred type and returns a primitive representation of the object, before falling back to using the object\'s valueOf() and toString() methods.',
	'Symbol.toStringTag': 'The Symbol.toStringTag static data property represents the well-known symbol Symbol.toStringTag. Object.prototype.toString() looks up this symbol on the this value for the property containing a string that represents the type of the object.',
	'Symbol.unscopables': 'The Symbol.unscopables static data property represents the well-known symbol Symbol.unscopables. The with statement looks up this symbol on the scope object for a property containing a collection of properties that should not become bindings within the with environment.',
	...
	'Temporal': 'The Temporal object enables date and time management in various scenarios, including built-in time zone and calendar representation, wall-clock time conversions, arithmetics, formatting, and more. It is designed as a full replacement for the Date object.',
	...
	'TypedArray': 'A _TypedArray_ object describes an array-like view of an underlying binary data buffer. There is no global property named TypedArray, nor is there a directly visible TypedArray constructor. Instead, there are a number of different global properties, whose values are typed array constructors for specific element types, listed below. On the following pages you will find common properties and methods that can be used with any typed array containing elements of any type.',
	'TypedArray.BYTES_PER_ELEMENT': 'The TypedArray.BYTES_PER_ELEMENT static data property represents the size in bytes of each element in a typed array.',
	'TypedArray.from': 'The TypedArray.from() static method creates a new typed array from an array-like or iterable object. This method is nearly the same as Array.from().',
	'TypedArray.of': 'The TypedArray.of() static method creates a new typed array from a variable number of arguments. This method is nearly the same as Array.of().',
	'URIError': 'The URIError object represents an error when a global URI handling function was used in a wrong way.',
	'Uint16Array': 'The Uint16Array typed array represents an array of 16-bit unsigned integers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Uint32Array': 'The Uint32Array typed array represents an array of 32-bit unsigned integers in the platform byte order. If control over byte order is needed, use DataView instead. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Uint8Array': 'The Uint8Array typed array represents an array of 8-bit unsigned integers. The contents are initialized to 0 unless initialization data is explicitly provided. Once established, you can reference elements in the array using the object\'s methods, or using standard array index syntax (that is, using bracket notation).',
	'Uint8Array.fromBase64': 'The Uint8Array.fromBase64() static method creates a new Uint8Array object from a base64-encoded string.',
	'Uint8Array.fromHex': 'The Uint8Array.fromHex() static method creates a new Uint8Array object from a hexadecimal string.',
	...
	'WeakRef': 'A WeakRef object lets you hold a weak reference to another object, without preventing that object from getting garbage-collected.',
	'WeakSet': 'A WeakSet is a collection of garbage-collectable values, including objects and non-registered symbols. A value in the WeakSet may only occur once. It is unique in the WeakSet\'s collection.',
	...
	'decodeURI': 'The decodeURI() function decodes a Uniform Resource Identifier (URI) previously created by encodeURI() or a similar routine.',
	...
	'escape': 'The escape() function computes a new string in which certain characters have been replaced by hexadecimal escape sequences.',
	...
	'globalThis': 'The globalThis global property contains the global this value, which is usually akin to the global object.',
	'isFinite': 'The isFinite() function determines whether a value is finite, first converting the value to a number if necessary. A finite number is one that\'s not NaN or ±Infinity. Because coercion inside the isFinite() function can be surprising, you may prefer to use Number.isFinite().',
	'isNaN': 'The isNaN() function determines whether a value is NaN, first converting the value to a number if necessary. Because coercion inside the isNaN() function can be surprising, you may prefer to use Number.isNaN().',
	...
	'unescape': 'The unescape() function computes a new string in which hexadecimal escape sequences are replaced with the characters that they represent. The escape sequences might be introduced by a function like escape().',
};
...
jush.api.php2 = jush.api.lowercase_keys({
	...
	'array_filter': '(array $array, ?callable $callback = null, int $mode = ?): array\nFilters elements of an array using a callback function',
	...
	'array_multisort': '(array &$array1, mixed $array1_sort_order = SORT_ASC, mixed $array1_sort_flags = SORT_REGULAR, mixed ...$rest): true\nSort multiple or multi-dimensional arrays',
	...
	'error_log': '(string $message, int $message_type = ?, ?string $destination = null, ?string $additional_headers = null): bool\nSend an error message to the defined error handling routines',
	...
	'file_get_contents': '(string $filename, bool $use_include_path = false, ?resource $context = null, int $offset = ?, ?int $length = null): string|false\nReads entire file into a string',
	'file_put_contents': '(string $filename, mixed $data, int $flags = ?, ?resource $context = null): int|false\nWrite data to a file',
	'file': '(string $filename, int $flags = ?, ?resource $context = null): array|false\nReads entire file into an array',
	...
	'forward_static_call_array': '(callable $callback, array $args): mixed\nCall a callback with an array of parameters, preserving the current called class',
	'forward_static_call': '(callable $callback, mixed ...$args): mixed\nCall a callback while preserving the current called class',
	...
	'json_decode': '(string $json, ?bool $associative = null, int $depth = 512, int $flags = ?): mixed\nDecodes a JSON string',
	...
	'ob_start': '(?callable $callback = null, int $chunk_size = ?, int $flags = PHP_OUTPUT_HANDLER_STDFLAGS): bool\nTurn on output buffering',
	...
	'stream_copy_to_stream': '(resource $from, resource $to, ?int $length = null, int $offset = ?): int|false\nCopies data from one stream to another',
	...
	'stream_socket_recvfrom': '(resource $socket, int $length, int $flags = ?, ?string &$address = null): string|false\nReceives data from a socket, connected or not',
	...
	'str_word_count': '(string $string, int $format = ?, ?string $characters = null): array|int\nReturn information about words used in a string',
	'strcasecmp': '(string $string1, string $string2): int\nBinary safe case-insensitive string comparison',
	'strchr': 'Alias of strstr',
	'strcmp': '(string $string1, string $string2): int\nBinary safe string comparison',
	'strcoll': '(string $string1, string $string2): int\nLocale based string comparison',
	'strcspn': '(string $string, string $characters, int $offset = ?, ?int $length = null): int\nFind length of initial segment not matching mask',
	...
	'strspn': '(string $string, string $characters, int $offset = ?, ?int $length = null): int\nFinds the length of the initial segment of a string consisting entirely of characters contained within a given mask',
	...
	'substr_count': '(string $haystack, string $needle, int $offset = ?, ?int $length = null): int\nCount the number of substring occurrences',
	...
	'Uri\\Rfc3986\\Uri::parse': '(string $uri, ?Uri\\Rfc3986\\Uri $baseUrl = null): ?static\nParse a URI',
	'Uri\\WhatWg\\Url::parse': '(string $uri, ?Uri\\WhatWg\\Url $baseUrl = null, array &$errors = null): ?static\nParse a URL',
	...
	'dba_open': '(string $path, string $mode, ?string $handler = null, int $permission = 0644, int $map_size = ?, ?int $flags = null): Dba\\Connection|false\nOpen database',
	'dba_optimize': '(Dba\\Connection $dba): bool\nOptimize database',
	'dba_popen': '(string $path, string $mode, ?string $handler = null, int $permission = 0644, int $map_size = ?, ?int $flags = null): Dba\\Connection|false\nOpen database persistently',
	...
	'FFI::string': '(FFI\\CData &$ptr, ?int $size = null): string\nCreates a PHP string from a memory area',
	...
	'iconv_mime_decode_headers': '(string $headers, int $mode = ?, ?string $encoding = null): array|false\nDecodes multiple MIME header fields at once',
	'iconv_mime_decode': '(string $string, int $mode = ?, ?string $encoding = null): string|false\nDecodes a MIME header field',
	...
	'iconv_strpos': '(string $haystack, string $needle, int $offset = ?, ?string $encoding = null): int|false\nFinds position of first occurrence of a needle within a haystack',
	...
	'imagechar': '(GdImage $image, GdFont|int $font, int $x, int $y, string $char, int $color): true\nDraw a character horizontally',
	...
	'imagestring': '(GdImage $image, GdFont|int $font, int $x, int $y, string $string, int $color): true\nDraw a string horizontally',
	...
	'IntlDateFormatter::formatObject': '(IntlCalendar|DateTimeInterface $datetime, array|int|string|null $format = null, ?string $locale = null): string|false\nFormats an object',
	...
	'grapheme_stripos': '(string $haystack, string $needle, int $offset = ?, string $locale = ""): int|false\nFind position (in grapheme units) of first occurrence of a case-insensitive string',
	...
	'grapheme_strpos': '(string $haystack, string $needle, int $offset = ?, string $locale = ""): int|false\nFind position (in grapheme units) of first occurrence of a string',
	'grapheme_strripos': '(string $haystack, string $needle, int $offset = ?, string $locale = ""): int|false\nFind position (in grapheme units) of last occurrence of a case-insensitive string',
	'grapheme_strrpos': '(string $haystack, string $needle, int $offset = ?, string $locale = ""): int|false\nFind position (in grapheme units) of last occurrence of a string',
	...
	'IntlChar::charAge': '(int|string $codepoint): ?array\nGet the "age" of the code point',
	'IntlChar::charDigitValue': '(int|string $codepoint): ?int\nGet the decimal digit value of a decimal digit character',
	'IntlChar::charDirection': '(int|string $codepoint): ?int\nGet bidirectional category value for a code point',
	...
	'IntlChar::charName': '(int|string $codepoint, int $type = IntlChar::UNICODE_CHAR_NAME): ?string\nRetrieve the name of a Unicode character',
	'IntlChar::charType': '(int|string $codepoint): ?int\nGet the general category value for a code point',
	'IntlChar::chr': '(int|string $codepoint): ?string\nReturn Unicode character by code point value',
	...
	'IntlChar::getBlockCode': '(int|string $codepoint): ?int\nGet the Unicode allocation block containing a code point',
	'IntlChar::getCombiningClass': '(int|string $codepoint): ?int\nGet the combining class of a code point',
	...
	'IntlChar::getIntPropertyValue': '(int|string $codepoint, int $property): ?int\nGet the value for a Unicode property for a code point',
	'IntlChar::getNumericValue': '(int|string $codepoint): ?float\nGet the numeric value for a Unicode code point',
	...
	'litespeed_request_headers': 'Alias of apache_request_headers',
	'litespeed_response_headers': 'Alias of apache_response_headers',
	...
	'pcntl_waitid': '(int $idtype = P_ALL, ?int $id = null, array &$info = [], int $flags = WEXITED, array &$resource_usage = []): bool\nWaits for a child process to change state',
	...
	'PDO::connect': '(string $dsn, ?string $username = null, ?string $password = null, ?array $options = null): static\nConnect to a database and return a PDO subclass for drivers that support it',
	...
	'Phar::createDefaultStub': '(?string $index = null, ?string $webIndex = null): string\nCreate a phar-file format specific stub',
	...
	'Phar::loadPhar': '(string $filename, ?string $alias = null): bool\nLoads any phar archive with an alias',
	'Phar::mapPhar': '(?string $alias = null, int $offset = ?): bool\nReads the currently executed file (a phar) and registers its manifest',
	...
	'Phar::webPhar': '(?string $alias = null, ?string $index = null, ?string $fileNotFoundScript = null, array $mimeTypes = [], ?callable $rewrite = null): void\nRoutes a request from a web browser to an internal file within the phar archive',
	...
	'socket_select': '(?array &$read, ?array &$write, ?array &$except, ?int $seconds, int $microseconds = ?): int|false\nRuns the select() system call on the given arrays of sockets with a specified timeout',
	...
	'gzopen': '(string $filename, string $mode, bool $use_include_path = false): resource|false\nOpen gz-file',
	...
	'inflate_init': '(int $encoding, array|object $options = []): InflateContext|false\nInitialize an incremental inflate context',
	...
	'Dom\\HTMLDocument::createFromFile': '(string $path, int $options = ?, ?string $overrideEncoding = null): Dom\\HTMLDocument\nParses an HTML document from a file',
	'Dom\\HTMLDocument::createFromString': '(string $source, int $options = ?, ?string $overrideEncoding = null): Dom\\HTMLDocument\nParses an HTML document from a string',
	...
	'ldap_exop': '(LDAP\\Connection $ldap, string $request_oid, ?string $request_data = null, ?array $controls = null, string &$response_data = ?, string &$response_oid = ?): LDAP\\Result|bool\nPerforms an extended operation',
	...
	'ldap_list': '(LDAP\\Connection|array $ldap, array|string $base, array|string $filter, array $attributes = [], int $attributes_only = ?, int $sizelimit = -1, int $timelimit = -1, int $deref = LDAP_DEREF_NEVER, ?array $controls = null): LDAP\\Result|array|false\nSingle-level search',
	...
	'ldap_read': '(LDAP\\Connection|array $ldap, array|string $base, array|string $filter, array $attributes = [], int $attributes_only = ?, int $sizelimit = -1, int $timelimit = -1, int $deref = LDAP_DEREF_NEVER, ?array $controls = null): LDAP\\Result|array|false\nRead an entry',
	...
	'ldap_search': '(LDAP\\Connection|array $ldap, array|string $base, array|string $filter, array $attributes = [], int $attributes_only = ?, int $sizelimit = -1, int $timelimit = -1, int $deref = LDAP_DEREF_NEVER, ?array $controls = null): LDAP\\Result|array|false\nSearch LDAP tree',
	...
	'mysqli_begin_transaction': '(mysqli $mysql, int $flags = ?, ?string $name = null): bool\nStarts a transaction',
	...
	'mysqli_commit': '(mysqli $mysql, int $flags = ?, ?string $name = null): bool\nCommits the current transaction',
	...
	'mysqli::poll': '(?array &$read, ?array &$error, array &$reject, int $seconds, int $microseconds = ?): int|false\nPoll connections',
	'mysqli_poll': '(?array &$read, ?array &$error, array &$reject, int $seconds, int $microseconds = ?): int|false\nPoll connections',
	'mysqli_prepare': '(mysqli $mysql, string $query): mysqli_stmt|false\nPrepares an SQL statement for execution',
	'mysqli_query': '(mysqli $mysql, string $query, int $result_mode = MYSQLI_STORE_RESULT): mysqli_result|bool\nPerforms a query on the database',
	'mysqli_real_connect': '(mysqli $mysql, ?string $hostname = null, ?string $username = null, ?string $password = null, ?string $database = null, ?int $port = null, ?string $socket = null, int $flags = ?): bool\nOpens a connection to the MySQL server',
	...
	'mysqli_rollback': '(mysqli $mysql, int $flags = ?, ?string $name = null): bool\nRolls back current transaction',
	...
	'openssl_cms_encrypt': '(string $input_filename, string $output_filename, OpenSSLCertificate|array|string $certificate, ?array $headers, int $flags = ?, int $encoding = OPENSSL_ENCODING_SMIME, string|int $cipher_algo = OPENSSL_CIPHER_AES_128_CBC): bool\nEncrypt a CMS message',
	'openssl_cms_read': '(string $input_filename, array &$certificates): bool\nExport the CMS file to an array of PEM certificates',
	'openssl_cms_sign': '(string $input_filename, string $output_filename, OpenSSLCertificate|string $certificate, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $private_key, ?array $headers, int $flags = ?, int $encoding = OPENSSL_ENCODING_SMIME, ?string $untrusted_certificates_filename = null): bool\nSign a file',
	...
	'openssl_csr_new': '(array $distinguished_names, ?OpenSSLAsymmetricKey &$private_key, ?array $options = null, ?array $extra_attributes = null): OpenSSLCertificateSigningRequest|bool\nGenerates a CSR',
	'openssl_csr_sign': '(OpenSSLCertificateSigningRequest|string $csr, OpenSSLCertificate|string|null $ca_certificate, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $private_key, int $days, ?array $options = null, int $serial = ?, ?string $serial_hex = null): OpenSSLCertificate|false\nSign a CSR with another certificate (or itself) and generate a certificate',
	'openssl_decrypt': '(string $data, string $cipher_algo, string $passphrase, int $options = ?, string $iv = "", ?string $tag = null, string $aad = ""): string|false\nDecrypts data',
	...
	'openssl_pkcs7_encrypt': '(string $input_filename, string $output_filename, OpenSSLCertificate|array|string $certificate, ?array $headers, int $flags = ?, int $cipher_algo = OPENSSL_CIPHER_AES_128_CBC): bool\nEncrypt an S/MIME message',
	...
	'openssl_private_decrypt': '(string $data, string &$decrypted_data, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $private_key, int $padding = OPENSSL_PKCS1_PADDING, ?string $digest_algo = null): bool\nDecrypts data with private key',
	...
	'openssl_public_encrypt': '(string $data, string &$encrypted_data, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $public_key, int $padding = OPENSSL_PKCS1_PADDING, ?string $digest_algo = null): bool\nEncrypts data with public key',
	...
	'openssl_sign': '(string $data, string &$signature, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $private_key, string|int $algorithm = OPENSSL_ALGO_SHA1, int $padding = ?): bool\nGenerate signature',
	...
	'openssl_verify': '(string $data, string $signature, OpenSSLAsymmetricKey|OpenSSLCertificate|array|string $public_key, string|int $algorithm = OPENSSL_ALGO_SHA1, int $padding = ?): int|false\nVerify signature',
	...
	'pg_copy_from': '(PgSql\\Connection $connection, string $table_name, array|Traversable $rows, string $separator = "\\t", string $null_as = "\\\\\\\\N"): bool\nInsert records into a table from an array',
	...
	'pg_execute': '(PgSql\\Connection $connection = ?, string $statement_name, array $params): PgSql\\Result|false\nSends a request to execute a prepared statement with given parameters, and waits for the result',
	...
	'pg_jit': '(?PgSql\\Connection $connection = null): array\nReturns the JIT information of the server',
	...
	'pg_put_copy_end': '(PgSql\\Connection $connection, ?string $error = null): int\nSignal the completion of a COPY operation to the server',
	...
	'pg_service': '(?PgSql\\Connection $connection = null): string\nGets the service name of the connection',
	...
	'pg_trace': '(string $filename, string $mode = "w", ?PgSql\\Connection $connection = null, int $trace_mode = ?): bool\nEnable tracing a PostgreSQL connection',
	...
	'simplexml_load_file': '(string $filename, ?string $class_name = SimpleXMLElement::class, int $options = ?, string $namespace_or_prefix = "", bool $is_prefix = false): SimpleXMLElement|false\nInterprets an XML file into an object',
	'simplexml_load_string': '(string $data, ?string $class_name = SimpleXMLElement::class, int $options = ?, string $namespace_or_prefix = "", bool $is_prefix = false): SimpleXMLElement|false\nInterprets a string of XML into an object',
	...
	'tidy::repairFile': '(string $filename, array|string|null $config = null, ?string $encoding = null, bool $useIncludePath = false): string|false\nRepair a file and return it as a string',
	...
	'odbc_columnprivileges': '(Odbc\\Connection $odbc, ?string $catalog, string $schema, string $table, string $column): Odbc\\Result|false\nLists columns and associated privileges for the given table',
	'odbc_columns': '(Odbc\\Connection $odbc, ?string $catalog = null, ?string $schema = null, ?string $table = null, ?string $column = null): Odbc\\Result|false\nLists the column names in specified tables',
	'odbc_commit': '(Odbc\\Connection $odbc): bool\nCommit an ODBC transaction',
	'odbc_connect': '(string $dsn, ?string $user = null, ?string $password = null, int $cursor_option = SQL_CUR_USE_DRIVER): Odbc\\Connection|false\nConnect to a datasource',
	...
	'odbc_foreignkeys': '(Odbc\\Connection $odbc, ?string $pk_catalog, string $pk_schema, string $pk_table, string $fk_catalog, string $fk_schema, string $fk_table): Odbc\\Result|false\nRetrieves a list of foreign keys',
	...
	'odbc_pconnect': '(string $dsn, ?string $user = null, ?string $password = null, int $cursor_option = SQL_CUR_USE_DRIVER): Odbc\\Connection|false\nOpen a persistent database connection',
	...
	'odbc_procedurecolumns': '(Odbc\\Connection $odbc, ?string $catalog = null, ?string $schema = null, ?string $procedure = null, ?string $column = null): Odbc\\Result|false\nRetrieve information about parameters to procedures',
	'odbc_procedures': '(Odbc\\Connection $odbc, ?string $catalog = null, ?string $schema = null, ?string $procedure = null): Odbc\\Result|false\nGet the list of procedures stored in a specific data source',
	...
	'odbc_statistics': '(Odbc\\Connection $odbc, ?string $catalog, string $schema, string $table, int $unique, int $accuracy): Odbc\\Result|false\nRetrieve statistics about a table',
	'odbc_tableprivileges': '(Odbc\\Connection $odbc, ?string $catalog, string $schema, string $table): Odbc\\Result|false\nLists tables and the privileges associated with each table',
	...
	'XMLReader::fromStream': '(resource $stream, ?string $encoding = null, int $flags = ?, ?string $documentUri = null): static\nCreates an XMLReader from a stream to read from',
	'XMLReader::fromString': '(string $source, ?string $encoding = null, int $flags = ?): static\nCreates an XMLReader from an XML string',
	'XMLReader::fromUri': '(string $uri, ?string $encoding = null, int $flags = ?): static\nCreates an XMLReader from a URI to read from',
	'XMLReader::open': '(string $uri, ?string $encoding = null, int $flags = ?): XMLReader\nSet the URI containing the XML to parse',
	'XMLReader::XML': '(string $source, ?string $encoding = null, int $flags = ?): XMLReader\nSet the data containing the XML to parse',
...
jush.api.php_new = jush.api.lowercase_keys({
	...
	'NoDiscard': '(?string $message = null)\nConstruct a new NoDiscard attribute instance',
	'Override': 'Construct a new Override attribute instance',
	'Closure': 'Constructor that disallows instantiation',
	'Error': '(string $message = "", int $code = ?, ?Throwable $previous = null)\nConstruct the error object',
	'ErrorException': '(string $message = "", int $code = ?, int $severity = E_ERROR, ?string $filename = null, ?int $line = null, ?Throwable $previous = null)\nConstructs the exception',
	'Exception': '(string $message = "", int $code = ?, ?Throwable $previous = null)\nConstruct the exception',
	...
	'Traversable': 'Abstract base interface that cannot be implemented alone',
	...
	'ValueError': 'A ValueError is thrown when the type of an argument is correct but its value is incorrect',
	'WeakMap': 'WeakMap implements ArrayAccess, Traversable (via IteratorAggregate), and Countable, so in most cases it can be used in the same fashion as an associative array',
	'DelayedTargetValidation': 'This attribute delays target validation errors for internal attributes from compile time to when the attribute is instantiated via the Reflection API',
	...
	'DateTime': '(string $datetime = "now", ?DateTimeZone $timezone = null)\nReturns new DateTime object',
	...
	'Random\\Randomizer': '(?Random\\Engine $engine = null)\nConstructs a new Randomizer',
	...
	'IteratorIterator': '(Traversable $iterator, ?string $class = null)\nCreate an iterator from anything that is traversable',
	...
	'SplFileObject': '(string $filename, string $mode = "r", bool $useIncludePath = false, ?resource $context = null)\nConstruct a new file object',
	...
	'Uri\\Rfc3986\\Uri': '(string $uri, ?Uri\\Rfc3986\\Uri $baseUrl = null)\nConstruct the Uri object',
	'Uri\\WhatWg\\InvalidUrlException': '(string $message = "", array $errors = [], int $code = ?, ?Throwable $previous = null)\nConstruct an InvalidUrlException object',
	'Uri\\WhatWg\\Url': '(string $uri, ?Uri\\WhatWg\\Url $baseUrl = null, array &$softErrors = null)\nConstruct the Url object',
	'Uri\\WhatWg\\UrlValidationError': '(string $context, Uri\\WhatWg\\UrlValidationErrorType $type, bool $failure)\nConstruct a UrlValidationError object',
	'Uri\\InvalidUriException': 'Indicates that a given URI is invalid or that an operation would result in an invalid URI',
	'Uri\\UriError': 'The base class for Errors that occur during processing of URIs',
	'Uri\\UriException': 'The base class for Exceptions that occur during processing of URIs',
	'BcMath\\Number': '(string|int $num)\nCreates a BcMath\\Number object',
	'com': '(string $module_name, array|string|null $server_name = null, int $codepage = CP_ACP, string $typelib = "")\ncom class constructor',
	'COMPersistHelper': '(?variant $variant = null)\nConstruct a COMPersistHelper object',
	...
	'finfo': '(int $flags = FILEINFO_NONE, ?string $magic_database = null)\nAlias finfo_open',
	'Filter\\FilterException': 'The base class for Exceptions thrown by the Filter extension',
	'Filter\\FilterFailedException': 'Thrown when a validation filter fails and the FILTER_THROW_ON_FAILURE flag is set',
	...
	'IntlListFormatter': '(string $locale, int $type = IntlListFormatter::TYPE_AND, int $width = IntlListFormatter::WIDTH_WIDE)\nCreates a new IntlListFormatter instance',
	...
	'UConverter': '(?string $destination_encoding = null, ?string $source_encoding = null)\nCreate UConverter object',
	...
	'PDO': '(string $dsn, ?string $username = null, ?string $password = null, ?array $options = null)\nCreates a PDO instance representing a connection to a database',
	'PDOException': 'Represents an error raised by PDO',
	'PDORow': 'Represents a row from a result set returned by PDOStatement::fetch called with PDO::FETCH_LAZY fetch mode',
	'PDOStatement': 'Represents a prepared statement and, after the statement is executed, an associated result set',
	'Phar': '(string $filename, int $flags = FilesystemIterator::SKIP_DOTS | FilesystemIterator::UNIX_PATHS, ?string $alias = null)\nConstruct a Phar archive object',
	'PharData': '(string $filename, int $flags = FilesystemIterator::SKIP_DOTS | FilesystemIterator::UNIX_PATHS, ?string $alias = null, int $format = ?)\nConstruct a non-executable tar or zip archive object',
	...
	'DOMElement': '(string $qualifiedName, ?string $value = null, string $namespace = "")\nCreates a new DOMElement object',
	...
	'Dom\\NamedNodeMap': 'Represents the set of attributes on an element',
	'Dom\\NamespaceInfo': 'This represents immutable information about namespaces of an element',
	'Dom\\Node': 'This is the modern, spec-compliant equivalent of DOMNode',
	'Dom\\NodeList': 'This is the modern, spec-compliant equivalent of DOMNodeList',
	...
	'mysqli_stmt': '(mysqli $mysql, ?string $query = null)\nConstructs a new mysqli_stmt object',
	...
	'SoapClient': '(?string $wsdl, array $options = [])\nSoapClient constructor',
	'SoapFault': '(array|string|null $code, string $string, ?string $actor = null, mixed $details = null, ?string $name = null, mixed $headerFault = null, string $lang = "")\nSoapFault constructor',
	...
	'SoapServer': '(?string $wsdl, array $options = [])\nSoapServer constructor',
	'SoapVar': '(mixed $data, ?int $encoding, ?string $typeName = null, ?string $typeNamespace = null, ?string $nodeName = null, ?string $nodeNamespace = null)\nSoapVar constructor',
	'Soap\\Sdl': 'A fully opaque class which replaces a soap_sdl resource as of PHP 8',
	'Soap\\Url': 'A fully opaque class which replaces a soap_url resource as of PHP 8',
	'SodiumException': 'Exceptions thrown by the sodium functions',
	'tidy': '(?string $filename = null, array|string|null $config = null, ?string $encoding = null, bool $useIncludePath = false)\nConstructs a new tidy object',
	'tidyNode': 'Private constructor to disallow direct instantiation',
	'Odbc\\Connection': 'A fully opaque class which replaces an odbc_connection resource as of PHP 8',
	'Odbc\\Result': 'A fully opaque class which replaces an odbc_result resource as of PHP 8',
